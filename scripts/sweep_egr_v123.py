...
BINS = {
    ...
    'v2_dep':     '/tmp/CHROMA_v2_clean',
    'v3_dep':     '/tmp/CHROMA_v3',
    'v3_raw':     '/tmp/CHROMA_v3_raw',
}
ARGS = {
    'baseline': ['-e', '0'],
    'v1':       ['--predict'],
    'v2_dep':   ['--predict', '--v2-model'],
    'v3_dep':   ['--predict', '--v2-model'],
    'v3_raw':   ['--predict', '--v2-model'],
}
...
for g in graphs:
    ...
    print(f'{g.name:32s} {flag} | base {fmt("baseline"):26s} | v1 {fmt("v1"):26s} | v2 {fmt("v2_dep"):26s} | v3 {fmt("v3_dep"):26s} | v3raw {fmt("v3_raw"):26s}',
          file=sys.stderr)

Path('model/v2_data/egr_v3_compare.json').write_text(json.dumps(rows, indent=2))
