...
CSRGraph::CSRGraph(const std::string &filename, bool single_edge = true) {
    if (filename.substr(filename.find_last_of(".") + 1) == "txt") {
        buildFromTxtFile(filename, single_edge);
...
/*  
    Only single edge for each edge, e.g. (1, 100) only 1's neighbor is 100. 100's neighbor is not 1
 
    - single_edge: CSRGraph to be single edge or double edge
*/
void CSRGraph::buildFromTxtFile(const std::string &filename, bool single_edge) {
    ...
    while (std::getline(file, line)) {
        ...
        sscanf(line.c_str(), "# Nodes: %d Edges: %d", &file_num_nodes, &file_num_edges);
        }
...
forward_flows = (int *)malloc(sizeof(int) * 2 * num_edges);
...
for (int i = 0; i < num_edges; ++i) {
    ...
    forward_flows[2*i] = graph.capacities[i]; // The initial residual flow is the same as capacity, not 0
    forward_flows[2*i + 1] = -1; // The index of the backward edge of (v, u)
}
...
bool CSRGraph::transfromToECLGraph(ECLgraph &g) {
    g.nodes = num_nodes;
    g.edges = num_edges;
    g.nindex = (int *)malloc(sizeof(int) * (num_nodes + 1));
    g.nlist = (int *)malloc(sizeof(int) * num_edges);
    g.eweight = (int *)malloc(sizeof(int) * num_edges);
    for (int i = 0; i < num_nodes + 1; ++i) {
        g.nindex[i] = offsets[i];
    }
    std::cout << "nindex finished" << std::endl;
    for (int i = 0; i < num_edges; ++i) {
        g.nlist[i] = destinations[i];
    }
    std::cout << "nlist finished" << std::endl;
    for (int i = 0; i < num_edges; ++i) {
        g.eweight[i] = 0;
    }
    std::cout << "eweight finished" << std::endl;
    return true;
}