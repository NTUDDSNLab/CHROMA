#!/usr/bin/env python3
"""
Batch runner for CPU Sequential Graph Coloring binaries.

Runs all cpu_* binaries in the current directory against all .egr datasets
in the specified directory. Logic similar to parallel script but:
1. No thread argument.
2. Supports --timeout.

Usage:
  python3 batch_cpu_seq_test.py -f <dataset_dir> -o <output.csv> [-r <runs>] [--timeout <sec>]
"""
...
def run_command(binary: str, dataset: str, timeout: Optional[float]) -> Tuple[Optional[float], Optional[int], bool, str]:
    """
    Runs the command: binary dataset
    ...
    try:
        ...
        return None, None, False, f"Timeout after {timeout}s"
...
def main():
    parser = argparse.ArgumentParser(description="Batch CPU Sequential Graph Coloring Runner")
    parser.add_argument("-f", "--dataset-dir", required=True, help="Directory containing .egr datasets")
    # No threads argument for sequential
    ...
    parser.add_argument("--timeout", type=float, default=None, help="Timeout in seconds per run")
    ...
    for binary in binaries:
        ...
        for dataset in datasets:
            ...
            for i in range(args.runs_per_benchmark):
                rt, col, success, err = run_command(binary, dataset, args.timeout)
                if success:
